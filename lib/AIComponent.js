...
function(
	...
	AIComponent.prototype.addBehavior = function(b, priority){
		if(null != this.list[b.name]){console.log(this); console.error("A name for the behavior:"+b.name+" already exists!");}
		var node = {
			next:null,
			previous:null,
		};
		...
		node.active = node.active == null ? true : node.active;
		node.blocking = node.blocking == null ? false : node.blocking;
		if(null != node.update){
	...
	AIComponent.prototype.destroy = function(){
		Game.unregister("Update", this, this.update);
		Game.unregister("FixedUpdate", this, this.fixedUpdate);
		Game.unregister("RenderUpdate", this, this.renderUpdate);
		Game.unregister("LateUpdate", this, this.lateUpdate);
	};
	...
	AIComponent.prototype.getBehaviorByName = function(name){
